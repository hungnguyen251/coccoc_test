<?php
require_once dirname(__DIR__, 1) . '/api/advert.php';

use PHPUnit\Framework\TestCase;

class AdvertTest extends TestCase
{
    private $ads;

    public function setUp()
    {
        parent::setUp();

        $this->ads = new Advert();
    }

    /**
     * @test
     */
    public function testGetAll()
    {
        $res = file_get_contents('http://localhost:8010/advert/get-list');
        $data = json_decode($res, true);

        //test http status
        $this->assertEquals(200, $data['code']);

        //test key data response
        $this->assertArrayHasKey('data', $data);
        
        //test data response exist title field or not
        $this->assertArrayHasKey('title', $data['data'][0]);

        //test data response exist link field or not
        $this->assertArrayHasKey('link', $data['data'][0]);

        //test data response exist price field or not
        $this->assertArrayHasKey('price', $data['data'][0]);
    }

    /**
     * @test
     */
    public function testGetById($id = 1)
    {
        $res = file_get_contents('http://localhost:8010/advert/' . $id);
        $data = json_decode($res, true);

        //test http status
        $this->assertEquals(200, $data['code']);

        //test data response exist title field or not
        $this->assertArrayHasKey('title', $data['data'][0]);

        //test data response exist link field or not
        $this->assertArrayHasKey('link', $data['data'][0]);

        //test data response exist price field or not
        $this->assertArrayHasKey('price', $data['data'][0]);
    }

    /**
     * @doesNotPerformAssertions
     */
    public function testValidateDataEmpty()
    {
        $data = [];
        $validate = $this->ads->validateData($data);
        // print_r($validate);

        //test data validate exist title field or not
        $this->assertArrayHasKey('title', $validate);

        //test data validate exist link field or not
        $this->assertArrayHasKey('price', $validate);

        //test data validate exist price field or not
        $this->assertArrayHasKey('link', $validate);
    }

    /**
     * @doesNotPerformAssertions
     */
    public function testValidateDataNotEmpty()
    {
        $data = [
            'title' => '',
            'price' => 100000,
            'link' => [
                'https://cdn.pixabay.com/photo/2015/04/23/22/00/tree-736885__480.jpg',
                'https://cdn.pixabay.com/photo/2015/04/23/22/00/tree-736885__481.jpg',
                'https://cdn.pixabay.com/photo/2015/04/23/22/00/tree-736885__482.jpg',
                'https://cdn.pixabay.com/photo/2015/04/23/22/00/tree-736885__483.jpg',
            ],
            'description' => [''],
        ];
        $validate = $this->ads->validateData($data);
        print_r($validate);

        //check the length of the title is more than 200 characters
        $this->assertArrayHasKey('title', $validate);

        //check the length of the description is more than 1000 characters
        $this->assertArrayHasKey('description', $validate);

        //check advert has more than 3 pictures
        $this->assertArrayHasKey('link', $validate);
    }

    /**
     * @test
     */
    public function testCreateAds()
    {
        $data = [
            'title' => 'test',
            'price' => 100000,
            'link' => ['https://cdn.pixabay.com/photo/2015/04/23/22/00/tree-736885__480.jpg'],
            'description' => ['image test'],
        ];

        $curl = curl_init();
        curl_setopt_array($curl, array(
            CURLOPT_RETURNTRANSFER => 0,
            CURLOPT_URL => 'http://localhost:8010/advert/create',
            CURLOPT_USERAGENT => 'application/json',
            CURLOPT_SSL_VERIFYPEER => false,
            CURLOPT_RETURNTRANSFER => true,
            CURLOPT_POSTFIELDS => json_encode($data), 
            CURLOPT_HTTP_VERSION => CURL_HTTP_VERSION_1_1,
            CURLOPT_CUSTOMREQUEST =>'POST',
            CURLOPT_HTTPHEADER => array(
                "cache-control: no-cache"
            ),
        ));
        $result = curl_exec($curl);
        $data = json_decode($result, true);

        curl_close($curl);

        print_r($data);
        
        //test http status
        $this->assertEquals(200, $data['code']);

        //test data response id
        $this->assertArrayHasKey('data', $data);

        //test data response id is an integer
        $this->assertInternalType('int', $data['data']);

    }

    /**
     * @test
     */
    public function testMethodCreatedFailed()
    {
        $data = [
            'title' => '',
            'price' => 100000,
            'link' => ['https://cdn.pixabay.com/photo/2015/04/23/22/00/tree-736885__480.jpg'],
            'description' => ['image test'],
        ];

        $curl = curl_init();
        curl_setopt_array($curl, array(
            CURLOPT_RETURNTRANSFER => 0,
            CURLOPT_URL => 'http://localhost:8010/advert/create',
            CURLOPT_USERAGENT => 'application/json',
            CURLOPT_SSL_VERIFYPEER => false,
            CURLOPT_RETURNTRANSFER => true,
            CURLOPT_POSTFIELDS => json_encode($data), 
            CURLOPT_HTTP_VERSION => CURL_HTTP_VERSION_1_1,
            CURLOPT_CUSTOMREQUEST =>'POST',
            CURLOPT_HTTPHEADER => array(
                "cache-control: no-cache"
            ),
        ));
        $result = curl_exec($curl);
        $data = json_decode($result, true);

        curl_close($curl);

        print_r($data);
        
        //test http status
        $this->assertEquals(400, $data['code']);
    }

    /**
     * @test
     */
    public function testMethodGetByIdlFailed()
    {
        $res = file_get_contents('http://localhost:8010/advert/');
        $data = json_decode($res, true);

        //test http status
        $this->assertEquals(404, $data['code']);
    }
}